...
#define MADCTL_ROTATE_180_DEGREES (MADCTL_COLUMN_ADDRESS_ORDER_SWAP | MADCTL_ROW_ADDRESS_ORDER_SWAP)
    ...
    printf("ST7789 Display Swap BGR");
    madctl ^= MADCTL_BGR_PIXEL_ORDER;
#endif
#if defined(DISPLAY_FLIP_ORIENTATION_IN_HARDWARE)
    printf("ST7789 DISPLAY_FLIP_ORIENTATION_IN_HARDWARE");
    ...
    printf("ST7789 DISPLAY_ROTATE_180_DEGREES");
    madctl ^= MADCTL_ROTATE_180_DEGREES;
#endif

#if defined(WAVESHARE_1INCH14_LCD) || defined(WAVESHARE_1INCH47_LCD) 

    madctl ^= MADCTL_ROTATE_180_DEGREES;
    madctl ^= MADCTL_ROW_COLUMN_EXCHANGE;
#endif

#if defined(WAVESHARE_1INCH9_LCD) 

    // madctl ^= MADCTL_ROTATE_180_DEGREES;
    madctl ^= MADCTL_ROW_COLUMN_EXCHANGE;
#endif
#if defined(WAVESHARE_1INCH69_LCD)
    madctl ^= MADCTL_ROTATE_180_DEGREES;
    // madctl ^= MADCTL_ROW_COLUMN_EXCHANGE;
#endif
    ...
    printf("ST7789 invertColors true\n");
    bool invertColors = true;
#else
    bool invertColors = false;
#endif
#ifdef DISPLAY_INVERT_COLORS
    printf("ST7789 DISPLAY_INVERT_COLORS")
    invertColors = !invertColors;
#endif

    if (invertColors) {
      printf("ST7789 display inversion ON\n");
